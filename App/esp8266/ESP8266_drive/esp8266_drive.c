...
{	
  u8 ucCh;
	...
	if(USART_GetITStatus( USART2, USART_IT_RXNE ) != RESET )
	{
		...
		//若末尾字符为x，这表示接受完成
		if(ucCh=='x') ESP8266_Fram_Record_Struct .InfBit .FramFinishFlag = 1;
...
{
	char cStr [20],dest [100];
	bool bRet = false;
	
	sprintf(dest,"AT+CIPSTART=0,\"%s\",\"%s\",%s\r\n","TCP",User_ESP8266_IP,User_ESP8266_PORT);
	ESP8266_Send_AT_Cmd ( dest, "OK", 0, 1000 );
