...
uint16_t FLASH_ReadID(void)
{
	...
	id |= SPI2_ReadWriteByte(0xFF) << 8; //读取16位ID 
...
void FLASH_ReadData(uint8_t *readBuff, uint32_t readAddr, uint16_t readByteNum)
{
	...
	/* 发送24位读取地址 */
	SPI2_ReadWriteByte(readAddr >> 16);
	SPI2_ReadWriteByte(readAddr >> 8);
	SPI2_ReadWriteByte(readAddr);

	/* 读取数据 */
	while(readByteNum--)
	{
		*readBuff = SPI2_ReadWriteByte(0xFF);
		readBuff++;
	}
...
{		
	...
	SPI2_ReadWriteByte(EN25X_SectorErase); //发送命令

	SPI2_ReadWriteByte(eraseAddr >> 16);  //发送24位地址
	SPI2_ReadWriteByte(eraseAddr >> 8);
	SPI2_ReadWriteByte(eraseAddr);
...
void FLASH_ChipErase(void)
{
	...
	SPI2_ReadWriteByte(EN25X_ChipErase); //发送命令
...
static void FLASH_WritePage(uint8_t *writeBuff, uint32_t writeAddr, uint16_t writeByteNum)
{
	...
	{  
		...
		SPI2_ReadWriteByte(EN25X_PageProgram); //发送写命令
	
		SPI2_ReadWriteByte(writeAddr >> 16);  //发送24位读取地址
		SPI2_ReadWriteByte(writeAddr >> 8);
		SPI2_ReadWriteByte(writeAddr);

	    for(i=0; i<byteNum; i++)				  //循环写数
		{
			SPI2_ReadWriteByte(*writeBuff);
			writeBuff++;	
		}
